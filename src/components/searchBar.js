...
const SearchBar = () => {
    ...
    if (lang !== "") {
      ...
      if (data.hasOwnProperty("items"))
        data.items.forEach((item) => {
          ...
          setUsers((arr) => [...arr, { user: usrName, url: item.html_url }]);
        });
      else
  ...
  const printResults = () => {
    if (lang === "" && result === false) return "";
    else if (lang === "" && result === true)
      return "Input the desired language.";
    else if (users.length > 0 && result === true) {
      var text = "Search";
      if (search_usr === true) text += " all users who have contributed to ";
      else text += " all repositories related to ";
      return (
        <SearchResult
          data={users}
          text={text + lang}
          title={search_usr ? "User" : "Repository"}
        />
      );
    }
  };
